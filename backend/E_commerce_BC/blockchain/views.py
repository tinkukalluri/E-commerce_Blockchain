...
htto_provider_url = r'https://sepolia.infura.io/v3/7ff8b75ef686485aa693b5bc3d6f7f4c'
web3 = Web3(Web3.HTTPProvider(htto_provider_url))
...
class VerifyPaymentRequest(APIView):
    def post(self, request):
        ...
        for event in events:
            ...
            if (order_id_ == order['id'] and to_address == contract_address and amount == order['order_total']):
                ...
                try:
                    ...
                    ShopOrderInstance.payment_status = api_models.PaymentStatus.objects.get(
                        id=1)
                    ShopOrderInstance.save(
                        update_fields=['transaction_hash', 'payment_status'])
                    ...
                    for orderline in order_line:
                        ...
                        productItemInstance.qty_in_stock = productItemInstance.qty_in_stock - \
                            int(qty_purchased)

                        try:
                            productItemInstance.save(
                                update_fields=['qty_in_stock'])
                            print(
                                '---------------new-product-quantity----------------')
                            print("productItem", productItemInstance.id)
                            print('qty_in_stock',
                                  productItemInstance.qty_in_stock)
                        except:
                            return Response({'status': False, 'oops': "looks like something went wrong with decrementing productItem quantity"}, status=status.HTTP_400_BAD_REQUEST
                                            )
                    return Response({
                        "status": True,
        ...
        return Response({
            ...
            "transaction_hash": ShopOrderInstance.transaction_hash,
            "payment_status": 3,
            "oops": "looks like we ran into problem please try again"
        }, status=status.HTTP_400_BAD_REQUEST)
